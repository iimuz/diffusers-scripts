...
def _main() -> None:
    ...
    log_settings.setup_lib_logger(filepath=log_filepath, loglevel=loglevel)
    log_settings.setup_logger(_logger, filepath=log_filepath, loglevel=loglevel)
    _logger.info(config)
    ...
    if config.device == _DeviceOption.CUDA:
        pipe_options["revision"] = "fp16"
        pipe_options["torch_dtype"] = torch.float16
    pipe = StableDiffusionPipeline.from_pretrained(model_id, **pipe_options)
